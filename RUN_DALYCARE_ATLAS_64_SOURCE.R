.dalycare_entry_path <- local({
  ...
  candidate <- file.path(getwd(), "RUN_DALYCARE_ATLAS_64_SOURCE.R")
...
if (!identical(basename(if (is.na(.dalycare_entry_path)) "" else .dalycare_entry_path), "RUN_DALYCARE_ATLAS_64_SOURCE.R") &&
    file.exists(file.path(getwd(), "RUN_DALYCARE_ATLAS_64_SOURCE.R"))) {
  .dalycare_entry_path <- normalizePath(file.path(getwd(), "RUN_DALYCARE_ATLAS_64_SOURCE.R"), winslash = "/", mustWork = FALSE)
...
cat("\nDALY-CARE Atlas one-click RStudio runner (64-resource restored validation map)\n")
cat("Project root: ", .dalycare_project_root, "\n", sep = "")
cat("Note: the restored validation map may contain more than 64 source-map rows because derived/helper rows are represented alongside canonical resources.\n")

.dalycare_source_map <- Sys.getenv("DALYCARE_ATLAS_SOURCE_MAP", unset = "config/source-map.dalycare64.restored.tsv")
