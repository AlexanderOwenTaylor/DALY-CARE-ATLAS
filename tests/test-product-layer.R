...
cll_raw <- raw_fields[raw_fields$source_name == "RKKP_CLL", , drop = FALSE]
expect_cll_raw <- function(raw_column, concept_id, variable_pattern) {
  rows <- cll_raw[cll_raw$raw_column == raw_column, , drop = FALSE]
  if (nrow(rows)) {
    expect_true(
      any(rows$clinical_concept_id == concept_id & grepl(variable_pattern, rows$clinical_variable, ignore.case = TRUE)),
      paste("CLL product raw field should map to", concept_id, "with expected label:", raw_column)
    )
  }
}
expect_cll_raw("Reg_BinetStadium", "binet_stage", "Binet stage")
expect_cll_raw("Reg_Umuteret", "ighv_mutation_status", "IGHV mutation status")
expect_cll_raw("Reg_FISH", "fish_availability", "FISH")
expect_cll_raw("Reg_Del17p", "del17p", "del\\(17p\\)")
expect_cll_raw("Reg_Del11q", "del11q", "del\\(11q\\)")
expect_cll_raw("Reg_Del13q14", "del13q14", "del\\(13q14\\)")
expect_cll_raw("Reg_Trisomi12", "trisomy12", "Trisomy 12")
expect_cll_raw("Reg_TP53", "tp53_status", "TP53 status")
expect_cll_raw("Beh_TP53Mutation", "tp53_status", "TP53 mutation")
expect_cll_raw("Reg_KnoglemarvsUndersoegelse", "bone_marrow_examination", "bone marrow")
expect_cll_raw("Reg_CTSCANNING", "cll_diagnostic_ct_workup", "CT workup")
expect_cll_raw("Reg_ULSCANNING", "cll_diagnostic_ultrasound_workup", "ultrasound workup")
expect_cll_raw("Beh_Vaegttab", "weight_loss", "Weight loss")
expect_cll_raw("Beh_Feber", "fever", "Fever")
expect_cll_raw("Beh_Nattesved", "night_sweats", "Night sweats")
expect_cll_raw("Beh_UdtaltTraethed", "marked_fatigue", "Marked fatigue")
expect_cll_raw("Beh_Lymfadenopati", "lymphadenopathy", "Lymphadenopathy")
expect_cll_raw("Beh_TargeteretBeh_Ibrutinib", "cll_targeted_therapy", "Ibrutinib")
expect_cll_raw("Beh_TargeteretBeh_venetoclax", "cll_targeted_therapy", "Venetoclax")
expect_cll_raw("Beh_TargeteretBeh_acalabrutinib", "cll_targeted_therapy", "Acalabrutinib")
expect_cll_raw("Beh_MRD", "mrd", "MRD")
expect_cll_raw("Beh_Responsevaluering", "response_evaluation", "Response evaluation")
expect_false(any(cll_raw$raw_column == "Beh_Vaegttab" & cll_raw$clinical_concept_id == "weight"), "CLL weight loss must not flow into product rows as weight measurement.")
