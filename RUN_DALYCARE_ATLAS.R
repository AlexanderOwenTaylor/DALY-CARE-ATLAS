.dalycare_entry_path <- local({
  ...
  candidate <- file.path(getwd(), "RUN_DALYCARE_ATLAS.R")
...
if (!identical(basename(if (is.na(.dalycare_entry_path)) "" else .dalycare_entry_path), "RUN_DALYCARE_ATLAS.R") &&
    file.exists(file.path(getwd(), "RUN_DALYCARE_ATLAS.R"))) {
  .dalycare_entry_path <- normalizePath(file.path(getwd(), "RUN_DALYCARE_ATLAS.R"), winslash = "/", mustWork = FALSE)
...
cat("\nDALY-CARE Atlas one-click RStudio runner\n")
